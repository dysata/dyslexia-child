...
1. Transcribing audio using dysata/Wav2Vec2-Ru-Child model
2. Aligning the transcription with a reference text using forced alignment
3. Marking discrepancies between reference and hypothesis with error codes ("marks")
4. Remapping reference segments to match hypothesis timing (improved alignment)

Library Usage:
from dysmarkccls_clean import get_markers
triples = get_markers(reference_text="hello world", audio_path="audio.wav")

CLI Usage:
# Simple mode (2 positional args)
python3 dysmarkccls_clean.py text.txt audio.wav
# Directory mode (flags)
python3 dysmarkccls_clean.py -a <audio_dir> -t <reference_text_file> -o <output_base>
"""
...
from w2vtransf import Segment2, Segment
import marks
...
MODEL_NAME_PROCESSOR = "dysata/Wav2Vec2-Ru-Child"
MODEL_NAME_MODEL = "dysata/Wav2Vec2-Ru-Child"
SAMPLING_RATE = 16000
...
def _load_model_cached() -> Tuple[Wav2Vec2Processor, Wav2Vec2ForCTC]:
    if _model_cache["processor"] is None or _model_cache["model"] is None:
        sys.stderr.write("Loading model (first use)...\n")
        processor = Wav2Vec2Processor.from_pretrained(MODEL_NAME_PROCESSOR)
        model = Wav2Vec2ForCTC.from_pretrained(MODEL_NAME_MODEL)
        model.eval()
...
# =============================================================================
def _get_emission_and_transcription(
    model: Wav2Vec2ForCTC,
    processor: Wav2Vec2Processor,
    audio: List[Any]
) -> Tuple[List[Any], str, torch.Tensor]:
    processed = processor(
        audio,
...
#        ).logits
    ...
    return emission_matrices, transcription, last_hidden_state
...
def _align_text_to_emission(
    ...
    text: str,
    processor: Wav2Vec2Processor
) -> Tuple[List[Any], List[int], List[Any], List[str]]:
    tokenizer = processor.tokenizer
    processedtxt = tokenizer(
        text=[text],
        padding=True,
        truncation=True,
        return_tensors="pt"
    ...
    trellis = w2vtr.get_trellis(emission_matrix, labels[0])
    path = w2vtr.backtrack(trellis, emission_matrix, labels[0])
    tokens = processor.tokenizer.convert_ids_to_tokens(
        labels[0],
        skip_special_tokens=False
    )
    segments = w2vtr.merge_repeats(path=path, tokenized=tokens)
    return segments, labels, path, tokens


def _create_comparison_segments(
    ref_segments: List[Any],
    hyp_segments: List[Any]
) -> Tuple[List[Segment2], List[Any]]:
    pointst1 = [seg.start for seg in ref_segments]
    pointsf2 = [seg.start for seg in hyp_segments]
    ...
    for i in points:
        if i in pointst1:
            wt1 = ref_segments[pointst1.index(i)].label
        if i in pointsf2:
            wf2 = hyp_segments[pointsf2.index(i)].label
        S2 = Segment2(wt1, wf2, '')
        segments2.append(S2)
    return segments2, points


def _apply_error_marks(segments: List[Segment2], points: List[Any], last_hidden_state) -> List[Segment2]:
...
def _format_output_data(segments: List[Segment2]) -> List[List[str]]:
    output_data = []
    for seg in segments:
...
def _process_audio_text_pair(
    ...
    emission_matrices, transcription, last_hidden_state = _get_emission_and_transcription(
        model, processor, sounds_in_batch
    )
    
    segmentst1, _, _, _ = _align_text_to_emission(
        emission_matrices[0], reference_text, processor
    )
    
    segmentsf2, _, _, _ = _align_text_to_emission(
        emission_matrices[0], transcription, processor
    )
    ...
    comparison_segments, points = _create_comparison_segments(segmentst1, segmentsf2)
    
    marked_segments = _apply_error_marks(comparison_segments, points, last_hidden_state)
