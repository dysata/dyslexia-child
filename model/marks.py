...
Порядок вызова: K - 7 - 1-6,8 - 9 - J - A - B - L - D - G - F - C - E - I - 0
...
def mark2(segments2):
    ...
    Returns:
        segments2: Same list with .mark updated
    """
    pair_set = {('н', 'п'), ('п', 'н'), ('н', 'и'), ('и', 'н')}

    for i in range(len(segments2)):
        if segments2[i].labelf2 != segments2[i].labelt1:
            if (segments2[i].labelt1, segments2[i].labelf2) in pair_set:
                if not segments2[i].mark:
...
def mark5(segments2):
    ...
    Returns:
        segments2: Same list with .mark updated
    """
    pair_set = {('р', 'ь'), ('ь', 'р'), ('б', 'ь'), ('ь', 'б'), ('б', 'р'), ('р', 'б')}
...
def mark9(segments2):
    ...
    for i in range(1, len(segments2) - 1):
        if segments2[i].labelf2 != segments2[i].labelt1:
            ...
            # Стечение согласных: t1 — согласная, заменена повтором предыдущей согласной
            elif (segments2[i].labelt1 in sogl and segments2[i].labelf2 in sogl and
                  i > 0 and segments2[i - 1].labelt1 in sogl and
                  segments2[i].labelf2 == segments2[i - 1].labelf2):
                if not segments2[i].mark:
                    segments2[i].mark = segments2[i].mark + '9'
    return segments2
                ...
                for k in range(i, j):
                    if segments2[k].labelf2 != segments2[k].labelt1:
                        ...
                        if segments2[k].labelt1 in vogel:  # Vowel skip gets markC
                            continue
                        # Стечение согласных: t1 — согласная после согласной → mark9, не K
                        if segments2[k].labelt1 in sogl and k > 0 and segments2[k - 1].labelt1 in sogl:
                            continue
...
def markD(segments2):
    ...
    Returns:
        segments2: Same list with .mark updated
    """
    # D приоритетнее всех других маркеров — перезаписывает любой
    for i in range(len(segments2) - 1):
        if segments2[i].labelf2 != segments2[i].labelt1 and \
           ...
           segments2[i].labelt1 in vogel:
            for j in range(i + 1, len(segments2)):
                if segments2[j].labelf2 == segments2[i].labelt1 and \
                   segments2[j].labelt1 == segments2[i].labelf2:
                    segments2[i].mark = 'D'
                    segments2[j].mark = 'D'
                    break
    return segments2
...
def markE(segments2):
    ...
    Returns:
        segments2: Same list with .mark updated
    """
    # E приоритетнее всех других маркеров — перезаписывает любой
    for i in range(len(segments2) - 1):
        if segments2[i].labelf2 != segments2[i].labelt1 and \
...
def markI(segments2, points, hidden_states):
    """
