...
var autoNextCounterInit = 15;
var autoNextCounter = autoNextCounterInit;
...
function startGame() {
    autoNext = document.getElementById('handsfree-mode').checked; 
    ...
    document.addEventListener('keyup', event => {
        if(!pageLocked) {
            if (event.code === 'Space') {
		if(autoNext){
			console.log("autoNextCounter = " + autoNextCounter);
			if(autoNextCounter>0){
				...
				if(!autoNextActive) {
					autoNextActive = true;
					alert("Переходим на автоматический режим листания. Нажмите мышкой ОК, чтобы закрыть сообщение, положите руки на колени и минимизируйте движения");
					for(let i = 0; i<autoNextCounterInit - 5; i++)
						meanEpochTime += epochEnd[i] - epochStart[i];
					meanEpochTime /= autoNextCounterInit - 5;

					console.log("Mean Epoch Time = " + meanEpochTime);
					console.log("epochStart.length = " + epochStart.length);
					console.log("epochEnd.length = " + epochEnd.length);

					btnclk();
				} else {
					alert("Листаем в автоматическом режиме. Нажмите мышкой ОК, чтобы закрыть сообщение, положите руки на колени и минимизируйте движения");
				}
			}
		} else {
			btnclk();
...
function wordProcess(){
    if (wordsRemain.length !== 0){       
	    ...
	    if(autoNextActive)
		setTimeout(btnclk, meanEpochTime);
        });
...
function btnclk(){
    ...
    //img.src = 'static/game1/images/cross.jpg';
...
function process(){
    if(!autoNextActive)
	epochStart.push(Date.now());
    while(!ready);
    if(init) {
        if(shownString != '') {
            if(!autoNextActive)
		epochEnd.push(Date.now());
            stop_recording({});
	}
    } else {
        init = true;
        ...
        if (picsRemain.length !== 0){
            ...
            if (ind !== -1 ) {
                ...
                start_recording(meta, function(){
                    ready = true;
                    const img = document.createElement('img');
                    img.src = 'static/game1/images/'+ (ind) + '.jpg';
                    img.style.height = '100%';
                    img.style.width = 'auto';
                    img.style.objectFit = 'contain';
                    document.getElementById('image-container').append(img);
